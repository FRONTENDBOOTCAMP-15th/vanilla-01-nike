# 1조: 코딩 네 스푼 🥄🥄🥄🥄

<hr/>

[![나이티 프로젝트 이미지](./public/images/imgNikeLogo.jpg)](https://cfs-nike1.netlify.app/)<br>⬆️ 이미지를 클릭하여 배포 사이트를 확인하세요.
<br>
<br>

# JUST DO IT!

### '코딩 네 스푼'은 글로벌 브랜드 나이키의 모바일 쇼핑 경험을 재현하여,

### 실제 구매자가 원하는 서비스를 제공하는 것을 목표로 프로젝트를 진행했습니다.

<br>
<br>

## 🕒 개발 기간

> 2025.11.10-2025.11.24 (총 15일)

<br>

## 👥 팀원 및 역할 소개

| <img src="./public/images/imgKHY.jpeg" width="150"> | <img src="./public/images/imgLHY.jpeg" width="150"> | <img src="./public/images/imgHJA.jpeg" width="150"> | <img src="./public/images/imgKHS.jpeg" width="150"> |
| :-------------------------------------------------: | :-------------------------------------------------: | :-------------------------------------------------: | :-------------------------------------------------: |
|                    **🐰 김하연**                    |                    **🍀 이하영**                    |                    **🐢 한정아**                    |                    **☃️ 김현수**                    |
|                      PM(팀장)                       |                         PL                          |                        서기                         |                        발표                         |
|                     상품 리스트                     |                      제품 상세                      |              메인 홈, 로그인&회원가입               |                      장바구니                       |

<br>

## 📢 발표 타임라인

#### **- 프로젝트 소개**

#### **- 기술 스택 및 개발 환경**

#### **- 기술 구현 상세 및 기능 시연**

#### **- 회고 및 추후 개선하고 싶은 점**

<br>
<br>
<br>
<br>

## 📝 프로젝트 소개

<hr>

### 프로젝트 선택 배경

이 프로젝트는 모바일 쇼핑의 일상화로 사용자 중심의 UI/UX의 중요성이 더욱 강조되는 시점에서,<br>글로벌 브랜드인 나이키의 사례를 바탕으로 직관적이고 효율적인 UI/UX 설계와 모바일 쇼핑 환경에 최적화된 기능을 구현하는데 목표를 두었습니다.

- 모바일 쇼핑의 일상화
- 사용자 중심 UI/UX 중요성 증가
- 글로벌 브랜드 사례를 통한 학습 필요성

### 학습한 점

- 사용자 편의성을 고려한 기능 구성: 효율적이고 사용자 중심의 인터페이스 설계
- 나이키의 직관적인 UI: 사용자에게 직관적이고 빠른 탐색 경험을 제공하는 디자인
- 모바일 중심의 구매 흐름: 모바일 쇼핑 환경에 최적화된 기능과 흐름 설계
- 컴포넌트 TS 파일 관리: 재사용성을 높임

<br>
<br>
<br>

## ⚙️ 기술 스택 및 개발 환경

<hr>

| 분류             | 툴                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **기술 스택**    | <img src="https://img.shields.io/badge/Tailwind-6d28d9?style=flat-square&logo=Tailwind&logoColor=white"/> <img src="https://img.shields.io/badge/Typescript-3178C6?style=flat-square&logo=Typescript&logoColor=white"/>                                                                                                                                                                                             |
| **UI/UX**        | <img src="https://img.shields.io/badge/Figma-F24E1E?style=flat-square&logo=Figma&logoColor=white"/>                                                                                                                                                                                                                                                                                                                 |
| **개발 환경**    | <img src="https://img.shields.io/badge/VisualStudioCode-007ACC?style=flat-square&logo=VisualStudioCode&logoColor=white"/> <img src="https://img.shields.io/badge/vite-646CFF?style=flat-square&logo=vite&logoColor=white"/>                                                                                                                                                                                         |
| **커뮤니케이션** | <img src="https://img.shields.io/badge/Github-181717?style=flat-square&logo=Github&logoColor=white"/> <img src="https://img.shields.io/badge/GIT-F05032?style=flat-square&logo=GIT&logoColor=white"/> <img src="https://img.shields.io/badge/Discord-5865F2?style=flat-square&logo=Discord&logoColor=white"/> <img src="https://img.shields.io/badge/notion-000000?style=flat-square&logo=notion&logoColor=white"/> |
| **배포**         | <img src="https://img.shields.io/badge/Netlify-01847B?style=flat-square&logo=Netlify&logoColor=white"/>                                                                                                                                                                                                                                                                                                             |

<br>
<br>
<br>

## 🖥️ 기술 구현 상세 및 기능 시연

<hr>

### 메인 홈

- **헤더:** 상품검색기능(작업미정) / 마이페이지→로그인 / 장바구니(링크) / 더보기-모달 창<br>
- **모달창:** 회원가입, 로그인 / 상품리스트, 장바구니 링크<br>
- **메인화면 구성 3가지:** 버튼 링크 연결 / 각 선택 가능 항목마다 hover기능 / 오버플로우 스트롤<br>
- **푸터:** 아코디언
  <br>
  <br>

### 로그인&회원가입

- **로그인:** 로그인 시 이메일 확인 → 회원일 시 비밀번호 입력 후 로그인 / 비회원일 시 비회원 문구 출력 후 회원가입(권한동의)<br>
- **회원가입:** 권한동의, 회원가입(필수 입력사항 미입력시 안넘어감)기능, 메인화면으로 이동<br>
- **권한 동의:** 모두 동의 박스 체크시 모든 input박스 체크 / 모든 박스 체크시 ‘계속’버튼 활성화 / 버튼 hover기능
  <br>
  <br>

### 상품 리스트

- **메인 카테고리별 상품 목록 조회:** 상단 햄버거 버튼을 누른 후 카테고리 버튼을 눌렀을 때도 누른 카테고리에 맞는 상품이 렌더링 됨.<br>
- **결과 텍스트 상품 개수와 연동:** 몇 개의 결과가 나오는지 알려주는 텍스트를 실제 리스트에 보이는 상품 개수와 동일하도록 만듦.<br>
- **페이지 처리 방식:** 스크롤 시 스크롤이 바닥에 닿았을 때 4개씩 추가로 상품이 더 보이도록 무한 스크롤 기능 추가
  <br>
  <br>

### 제품 상세

- **사이즈 선택 + 재고여부 확인:** 사이즈 선택 기능 연결 완료<br>
- **장바구니에 담기:** 로그인 시: 사이즈 선택 후 장바구니 추가 기능 연결 완료 / 비로그인 시: 로그인이 필요하다는 문구 출력<br>
- **위시리스트 추가:** 위시리스트 추가 기능 연결 완료<br>
- **추천 제품 목록:** 현재 보고 있는 제품의 카테고리와 일치하는 제품을 추천하는 기능 연결 완료
  <br>
  <br>

### 장바구니

- **장바구니 상품 목록 조회:** 장바구니에 담겨져 있는 상품의 목록들을 조회<br>
- **수량 수정:** 플러스, 마이너스를 버튼을 클릭 시 상품의 수량과 총 금액을 변경<br>
- **찜하기:** 빈 하트 아이콘 클릭 시 채워진 하트로 변경<br>
- **삭제:** 휴지통 아이콘 클릭 시 장바구니에서 해당 상품을 삭제
  <br>
  <br>

<br>

## 💡 회고 및 추후 보완하고 싶은 점

<hr>

| **팀원**      | **회고**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | **추후 개선하고 싶은 점**                                                                                                                                                                                                                                                                                        |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **🐰 김하연** | 내가 얼마나 개발 지식이 부족한지 뼈저리게 깨달은 시간이었다. 수업 내용을 잘 따라간 것과 프로젝트를 할 수 있는 건 완전히 다른 문제였다. 인터넷에서 많은 자료를 검색하고 공부해 가면서 작업을 하느라 시간이 꽤나 소요되었던 것 같다. 그 덕에 전보다 개발 지식이 훨씬 넓어졌음에 뿌듯함을 느끼고 있다. 한 걸음 더 성장할 수 있어 감사한 시간이었다.                                                                                                                                                             | 필터 체크박스를 종류별로 나누지 않고 쭉 나열했는데, 보기에 좋지 않은 것 같아서 종류별로 나누는 걸로 수정하고 싶다. 중요도가 낮아서 고치지 않은 게 있었는데 상품 리스트에서 가격의 위치 높낮이가 전부 다른데, 이것도 보기 좋게 맞추고 싶다. 이후에도 개선해야 할 사항들을 고쳐서 좀 더 나은 결과물을 만들 것이다. |
| **🍀 이하영** | 2주라는 짧은 기간 동안 프로젝트를 진행하며 배운 내용을 활용해볼 수 있어서 좋은 경험이었습니다. 완벽하게 마무리하진 못했지만, 추후 개선을 통해 완성도를 높일 계획입니다.                                                                                                                                                                                                                                                                                                                                      | 마크업, 코드 개선 및 리뷰 API 추가                                                                                                                                                                                                                                                                               |
| **🐢 한정아** | 이번 바닐라 프로젝트를 진행하면서 내 생각보다 나는 내가 배운것을 잘 소화하고 있지 못했다는 것을 깨달았다. 필요한 기능을 다 알고 적재적소에 사용할 수 있다고 생각했는데 막상 내가 코드를 짤 떄에는 막막해서 많이 헤메었다.하나의 프로젝트를 진행하는데 있어 나 혼자는 역부족이지만, 팀원들과 함께 프로젝트를 진행하게 되어 즐겁고 고마운 시간이었다.이번 바닐라 프로젝트를 통해 개발에 대한 생각의 폭이 달라진것같고, 부족한 부분은 급하게 생각하지 말고 다만 최선을 다해서 메꿔나가야겟다고 다짐하게 되었다. | 로그인 api 구현과 header의 장바구니갯수 api 구현으로 사용자 편의성을 개선하고싶다.                                                                                                                                                                                                                               |
| **☃️ 김현수** | 기획 단계부터 어느 부분이 많은 시간을 소요하는 지에 대해 명확히 판단하고 작업해야겠다는 생각이 들었다. 프로젝트가 끝나갈수록 각종 오류와 싸우느라 API 연결을 하지 못한 것이 매우 아쉽고, 아직 개발자로 성장하기에는 부족하다는 것을 많이 느꼈다.                                                                                                                                                                                                                                                             | API를 연결해 프로젝트를 마무리 짓고 싶다.                                                                                                                                                                                                                                                                        |
