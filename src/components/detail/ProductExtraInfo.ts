class ProductExtraInfo extends HTMLElement {
  ...
  render() {
    this.innerHTML = `
    <div class="w-[312px] flex flex-col mx-6 text-xl">
      <!-- 무료 배송 및 반품 섹션 -->
      <section>
        <button
          type="button"
          aria-expanded="false"
          aria-controls="shippingPanel"
        ...
        <div id="shippingPanel" hidden>
          ...
          <p class="text-sm">오늘도착 서비스</p>
          <ul
            class="flex flex-col gap-3 list-disc list-outside pl-7 my-3 text-xs"
          >
            <li>
              이용시간: 오전 10시 30분까지 결제 시, 당일 도착 (일요일, 공휴일
              제외)
            </li>
            <li>
      ...
      <!-- 리뷰 섹션 -->
      <section class="border-y border-[#e5e5e5]">
        <button
          type="button"
          aria-expanded="false"
          aria-controls="reviewPanel"
          class="flex justify-between cursor-pointer w-full my-7.5"
        >
          리뷰 (2)
          <svg
...
customElements.define('product-extrainfo', ProductExtraInfo);
